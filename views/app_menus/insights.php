...
//report 4 data
$report4Data = FSwpDB()->get_results("SELECT CONCAT(node_id,'_',node_type) AS node , SUM(visit_count) AS c FROM ".FSwpTable('feeds')." tb1 WHERE ( (node_type='account' AND (SELECT COUNT(0) FROM ".FSwpTable('accounts')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId' OR tb2.is_public=1))>0) OR (node_type<>'account' AND (SELECT COUNT(0) FROM ".FSwpTable('account_nodes')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId')>0 OR tb2.is_public=1)) ) GROUP BY CONCAT(node_id,'_',node_type) ORDER BY c DESC LIMIT 0,10" , ARRAY_A);
$report4 = [
	'data' => [],
	'labels' => []
];
foreach($report4Data AS $r4Data)
{
	...
	$nodeInf = FSwpFetch($nodeInfTable , $nodeId);
	...
	$report4['labels'][] = esc_html(ucfirst($nodeInf['driver']) . ' / ' . FScutText($nodeInf['name'] , 20));
}

$monthStart = date('Y-m-01');
$monthEnd = date('Y-m-d');
$sharesThisMonth = FSwpDB()->get_row("SELECT COUNT(0) AS c FROM ".FSwpTable('feeds')." tb1 WHERE is_sended='1' AND status='ok' AND CAST(send_time AS DATE) BETWEEN '$monthStart' AND '$monthEnd' AND ( (node_type='account' AND (SELECT COUNT(0) FROM ".FSwpTable('accounts')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId' OR tb2.is_public=1))>0) OR (node_type<>'account' AND (SELECT COUNT(0) FROM ".FSwpTable('account_nodes')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId')>0 OR tb2.is_public=1)) )" , ARRAY_A);

$hitsThisMonth = FSwpDB()->get_row("SELECT SUM(visit_count) AS c FROM ".FSwpTable('feeds')." tb1 WHERE is_sended='1' AND status='ok' AND CAST(send_time AS DATE) BETWEEN '$monthStart' AND '$monthEnd' AND ( (node_type='account' AND (SELECT COUNT(0) FROM ".FSwpTable('accounts')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId' OR tb2.is_public=1))>0) OR (node_type<>'account' AND (SELECT COUNT(0) FROM ".FSwpTable('account_nodes')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId')>0 OR tb2.is_public=1)) )" , ARRAY_A);

$accounts = FSwpDB()->get_row(FSwpDB()->prepare( "SELECT COUNT(0) AS c FROM ".FSwpTable('accounts') . " tb1 WHERE is_public=1 OR user_id=%d", [get_current_user_id()] ) , ARRAY_A);

$nodes = FSwpDB()->get_row(FSwpDB()->prepare("SELECT COUNT(0) AS c FROM ".FSwpTable('account_nodes') . " tb1 WHERE is_public=1 OR user_id=%d", [get_current_user_id()]) , ARRAY_A);

$hitsThisMonthSchedule = FSwpDB()->get_row("SELECT SUM(visit_count) AS c FROM ".FSwpTable('feeds')." tb1 WHERE is_sended='1' AND status='ok' AND CAST(send_time AS DATE) BETWEEN '$monthStart' AND '$monthEnd' AND schedule_id>0 AND ( (node_type='account' AND (SELECT COUNT(0) FROM ".FSwpTable('accounts')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId' OR tb2.is_public=1))>0) OR (node_type<>'account' AND (SELECT COUNT(0) FROM ".FSwpTable('account_nodes')." tb2 WHERE tb2.id=tb1.node_id AND (tb2.user_id='$userId')>0 OR tb2.is_public=1)) )" , ARRAY_A);
?>
