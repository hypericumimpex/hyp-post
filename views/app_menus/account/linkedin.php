...
$accountsList = wpDB()->get_results(wpDB()->prepare("
	SELECT 
	 	*,
		(SELECT COUNT(0) FROM ".wpTable('account_nodes')." WHERE account_id=tb1.id) AS companies,
		(SELECT filter_type FROM ".wpTable('account_status')." WHERE account_id=tb1.id AND user_id=%d) is_active
	FROM ".wpTable('accounts')." tb1
	WHERE (user_id=%d OR is_public=1) AND driver='linkedin'",  [get_current_user_id(), get_current_user_id()]) , ARRAY_A);

?>

<div style="margin: 40px 80px;">
	<span style="color: #888; font-size: 17px; font-weight: 600; line-height: 36px;"><span id="accounts_count"><?php print count($accountsList);?></span> <?=esc_html__('linkedin account added', 'fs-poster')?></span>
...
<div class="ws_table_wraper">
	<table class="ws_table">
		...
		<tr>
			...
			<th><?=esc_html__('COMPANY PAGES', 'fs-poster')?></th>
		...
		foreach($accountsList AS $accountInf)
		{
			...
			<tr data-id="<?=$accountInf['id']?>">
				...
				<td class="ws_tooltip" data-title="<?=esc_html__('Click to see list', 'fs-poster')?>" data-load-modal="show_nodes_list" data-parameter-account_id="<?=$accountInf['id']?>">
					<i class="ws_icon_style fa fa-building"></i> <?=esc_html($accountInf['companies']);?>
					<?=esc_html__('company', 'fs-poster')?>
				</td>
				<td>
