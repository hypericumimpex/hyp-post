...
$accountsList = wpDB()->get_results(wpDB()->prepare("
	SELECT 
		*,
		(SELECT COUNT(0) FROM ".wpTable('account_nodes')." WHERE account_id=tb1.id) communities,
		(SELECT filter_type FROM ".wpTable('account_status')." WHERE account_id=tb1.id AND user_id=%d) is_active 
	FROM ".wpTable('accounts')." tb1 
	WHERE (user_id=%d OR is_public=1) AND driver='google'",  [get_current_user_id(), get_current_user_id()]) , ARRAY_A);

?>

<div style="margin: 40px 80px;">
	<span style="color: #888; font-size: 17px; font-weight: 600; line-height: 36px;"><span id="accounts_count"><?php print count($accountsList);?></span> <?=__('google account added', 'fs-poster')?></span>
	<button type="button" class="ws_btn ws_bg_dark" style="float: right;" data-load-modal="add_google_account_methods"><i class="fa fa-plus"></i> <?=esc_html__('ADD ACCOUNT', 'fs-poster')?></button>
</div>
	...
	<table class="ws_table">
		...
		foreach($accountsList AS $accountInf)
		{
			...
			<tr data-id="<?=$accountInf['id']?>">
				...
				<td class="ws_tooltip" data-title="<?=esc_html__('Click to see list' , 'fs-poster')?>" data-load-modal="google_show_communities_list" data-parameter-account_id="<?=$accountInf['id']?>"><i class="ws_icon_style fa fa-users"></i> <?=$accountInf['communities']?> <?=esc_html__('communities' , 'fs-poster')?></td>
				<td>
